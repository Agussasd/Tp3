...
def orden_topologico_dfs(grafo, v, pila, visitados):from grafo import Grafo
from tdas import Cola, Pila
from heapq import heapify, heappush, heappop #Heappush encola, heappop desencola del heap
import random
...
def ciclo_vacaciones(grafo, origen, visitados, camino, n, corte):
	if corte > n:
		return False
	camino.append(origen)
	visitados.add(origen)
	for v in grafo.adyacentes(origen):
		if v in visitados and camino[0] == v and len(visitados) == n:
			camino.append(v)
			return True
		if v not in visitados:
			if ciclo_vacaciones(grafo, v, visitados, camino, n, corte + 1):
				return True
